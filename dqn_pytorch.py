...
def select_action(state,epsilon):
    ...
    if sample > eps_threshold:
        return model(
            Variable(state, volatile=True).type(FloatTensor)).data.max(1)[1].view(1, 1)[0,0]
    else:
...
def playGame():
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % FRAME_PER_ACTION == 0:
            a_t = select_action(state,epsilon)
        else:
            a_t = LongTensor([[0]]) # do nothing
        ...
        x_t1_colored, r_t, terminal = game_state.frame_step(a_t[0, 0])
